...
// app.get('/confirm', mail.confirm)
// app.get('/del', mail.del)
app.get('/status', ser.status)

sockets = []
chats = []

//Lấy tất cả bài posts để tăng hiệu suất khi người dùng tuy cập trang post
posts_public = {}
...
getHistoryChat()
getPostsPublic()
//Lấy thời gian trước đó có người gửi tin nhắn xác thực
history_req = {}

//Lấy lịch sử chat trước đo khi vừa khởi động server
const chatRoom = 'chat_room_15082000'
io.on('connection', (socket) => {

    console.log(`User connected ${socket.id}`)
    //trả về lịch sử chat trước đó cho client mới đăng nhập
    //Thêm bài viết mới
    socket.on('add_post', (data, callback) => {
        try {
            ...
            if (user_id) {
                ...
                action.then((res) => {
                    //Nếu bài việc được đăng dưới dạng chỉ mình tôi thì không gửi cho ai khác
                    if (data.share !== 2) {
    ...
    //Tìm kiếm
    // SELECT * FROM posts WHERE posts.title LIKE '%a b c%'
    ...
    //Thay đổi phạm vi chia sẻ của bài viết công khai- bạn bè- chỉ mình tôi
