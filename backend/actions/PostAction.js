...
module.exports = {
    save: (data, user_id) => {
        return new Promise(async (resolve) => {
            ...
            const sql = "INSERT INTO posts_save (user_id, title, content) VALUES (?, ?, ?)"
    ...
    add: (req, user_id, user_name, user_image) => {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                const sql = "INSERT INTO posts (user_id, content, title, url, share) VALUES (?, ?, ?, ?, ?)"
                const title_url = titleURL(title)
                db.query(sql, [user_id, content, title, title_url, share], function (err, result) {
    ...
    edit: (data, user_id) => {
        return new Promise(async (resolve) => {
            try {
                ...
                const sql = 'UPDATE posts SET content = ?, title = ?, url = ? WHERE id = ? AND user_id = ?'
                db.query(sql, [content, data.title, title_url, data.id, user_id], (err) => {
                    if (err) return resolve(false)
                    return resolve(true)
                })
    ...
    editSave: (data, user_id) => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                content = jwt.sign({ content }, 'shhhhh')
                const title_url = titleURL(data.title)
                const sql = 'UPDATE posts_save SET content = ?, title = ?WHERE id = ? AND user_id = ?'
    ...
    get: (topic, _user_id) => {
        return new Promise((resolve, reject) => {
            try {
                let login = false
                //Không tồn tại token hoặc token fail thì get sản phẩm không có token
                let user_id = -1
                if (_user_id) {
                    user_id = _user_id
                    login = true
                ...
                //Tiến hành kiểm tra những bài viết đã like
                const sql = 'SELECT post_id FROM post_like WHERE user_id = ?'

                db.query(sql, [user_id], (err, n_like) => {
                    if (err) return reject(err)
                    //true: chưa like bài nào và ngược lại
                    const check_like = (n_like.length !== 0)
                    //Lấy thông tin bài viết
                    const sql = 'SELECT * FROM account INNER JOIN posts ON posts.user_id = account.id WHERE posts.share = 0 ORDER BY posts.date DESC'
                            ...
                            return (
                                {
                                    ...
                                    //Kiểm tra xem người dùng có like bài viết này không
                                    'user_like': check_like === false ? false :
                                        (n_like.find(item => item.post_id === value.id) !== undefined)
                                }
                            )
                        })
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                db.query(sql, [post_id], (err, reponse) => {
                    ...
                    const sql = 'SELECT * FROM post_like WHERE user_id = ? AND post_id = ? LIMIT 1'
                    db.query(sql, [user_id, post_id], (err, reponse) => {
                        if (err) return reject(err)

                        reponse = reponse[0]
                        //Nếu chưa like sẽ tiến hành like
                        if (reponse === undefined) {
                            const sql = 'UPDATE posts SET n_like = n_like + 1 WHERE id = ?'
                            db.query(sql, [post_id], (err) => {
                                if (err) return reject(err)
                                //Thêm
                                const sql = 'INSERT INTO post_like (user_id, post_id) VALUES (?, ?)'
                            ...
                            db.query(sql, [post_id], () => {
                                //Xóa
                                const sql = 'DELETE FROM post_like WHERE user_id = ? AND post_id = ?'
                                db.query(sql, [user_id, post_id], (err) => {
                                    if (err) return reject(err)
                                    return resolve({
                                        "like": false,
    ...
    detail: async (post_id, post_url, user_id) => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const sql = 'SELECT * FROM account INNER JOIN posts ON posts.user_id = account.id WHERE posts.id = ? LIMIT 1'
                db.query(sql, [post_id], function (err, reponse) {
                    if (err) return reject(err)
                    reponse = reponse[0]

                    if (!reponse) return resolve()
                    // Bài viết chia sẻ riêng tư
                    if (reponse.share === 2 && user_id !== reponse.user_id) return resolve(2)
                    //Bài viết chia sẻ cho bạn bè
                    if (reponse.share === 1 && user_id !== reponse.user_id) {
                        const sql = "SELECT * FROM friend WHERE (user_id = ? AND user_id_2 = ?) OR (user_id = ? AND user_id_2 = ?) LIMIT 1"
                        db.query(sql, [user_id, reponse.user_id, reponse.user_id, user_id], (err, friend) => {
                            if (err) return reject(err)
                            friend = friend[0]
                            // Không phải là bạn bè
                            if (!friend) return resolve(1)
                            //Nếu link không có url hoặc url sai thì chuyển lại đúng định dạng
                            navigation = false
                            //true sẽ chuyển hướng sang link đúng
                            if (reponse.url !== post_url) navigation = true


                            //Lấy tất cả bình luận có trong bài viết
                            ...
                            db.query(sql, [reponse.id], async (err, user) => {
                                ...
                                return resolve({
                                    ...
                                    "data": {
                                        ...
                                        'date': moment(reponse.date).format('DD-MM-YYYY') + " lúc " + moment(reponse.date).format('HH:mm'),
    ...
    trend: async () => {
        return new Promise((resolve) => {
            // const sql = 'SELECT * FROM  posts ORDER BY n_like + n_comments DESC LIMIT 6'
            const sql = 'SELECT * FROM account INNER JOIN posts WHERE posts.user_id = account.id ORDER BY n_like + n_comments DESC LIMIT 6'
    ...
    postsOfUser: (user_id, user_id_post) => {
        return new Promise(async (resolve, reject) => {
            try {
                //Kiểm tra xem người dùng này có ẩn bài viết hay không
                const sqll = "SELECT posts FROM private WHERE user_id = ?"
                db.query(sqll, [user_id_post], (err, res) => {
                    if (err) return reject(err)
                    const show = (res[0].posts === 1)

                    //Người dùng này cho phép hiển thị
                    if (show) {
                        //Tiến hành kiểm tra những bài viết đã like
                        const sql = 'SELECT post_id FROM post_like INNER JOIN posts ON post_like.post_id = posts.id WHERE posts.user_id = ? AND post_like.user_id = ?'
                        db.query(sql, [user_id_post, user_id], (err, res) => {
                            ...
                            //Lấy thông tin bài viết
                            const sql = 'SELECT * FROM account INNER JOIN posts ON posts.user_id = account.id WHERE posts.user_id = ? ORDER BY posts.date DESC'
                            db.query(sql, [user_id_post], (err, res) => {
                                ...
                                if (res.length !== 0) {
                                    res.map((value) => {
                                        //Kiểm tra xem phải hôm nay hay không
                                        data.push({
                                            'id': value.id,
                                            'name': value.title,
                                            'user_id': value.user_id,
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                //Nếu là bạn bè
                    ...
                    //Lấy tất cả bạn bè
                    const sql = "SELECT * FROM friend WHERE (friend.user_id = ? OR friend.user_id_2 = ?) AND friend.status = 1"
                    db.query(sql, [user_id, user_id], (err, res) => {
                        if (err) return reject(err)
                        friend = [user_id]
                        res.map((value) => {
                            if (value.user_id === user_id)
                                friend.push(value.user_id_2)
                            else
                                friend.push(value.user_id)
                        })
                        const sql = `SELECT * FROM account INNER JOIN posts ON posts.user_id = account.id 
                        ...
                        db.query(sql, [text], (err, res) => {
                            ...
                            res.map((value) => {
                                ...
                                if (value.user_id === user_id)
                                    _type = 0
                                else if (friend.indexOf(value.user_id) !== -1)
                                    _type = 1
    ...
    //Lấy tất cả bài viết (setup)
    getPostPublic: () => {
        return new Promise((resolve, reject) => {
            try {

                const sql = 'SELECT * FROM account INNER JOIN posts ON posts.user_id = account.id WHERE posts.share = 0 ORDER BY posts.date'
                ...
                db.query(sql, async function (err, reponse) {
                    ...
                    const get_all = reponse.map(async (value) => {
                        //Lấy tất cả người dùng like bài viết này
