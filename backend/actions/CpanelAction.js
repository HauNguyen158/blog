...
const getBand = (user_id, value) => {
    return new Promise((resolve, reject) => {
        ...
        db.query(sql, [user_id], async (err, res) => {
            ...
            const status = band !== false ? "pending" :
                value === 1 ? "completed" : "failed"
    ...
    editUser: (data) => {
        return new Promise((resolve, reject) => {
            try {
                ...
                const sql = "SELECT * FROM  account WHERE username = ? AND id <> ? LIMIT 1"
                db.query(sql, [username, user_id], (err, res) => {
                    ...
                    const sql = "UPDATE account SET username = ?, name = ?, email = ?, status = ? WHERE id = ?"
                    db.query(sql, [username, data.name, data.email, data.status === 'completed' ? 1 : 0, user_id], (err, res) => {
                        if (err) return reject(err)
                        return resolve(true)
                    })
                })
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                db.query(sql, [user_id], (err) => {
                    ...
                    //Tiến hành khóa
                    if (reason !== '') {
                        const sql = "INSERT INTO ban (user_id, time, reason) VALUES (?, ?, ?)"
