...
module.exports = {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (username === undefined || password === undefined
                    || email === undefined || _name === undefined)
                ...
                db.query(sql, [username], function (err, reponse) {
                    ...
                    const sql = "INSERT INTO account (username, password, email, name) VALUES (?, ?, ?, ?)"
                    db.query(sql, [username, md5(password), email, _name], function (err, res) {
                        if (err) return reject(false)
    ...
    login: (data) => {
        return new Promise((resolve) => {
            try {
                ...
                const sql = 'SELECT * FROM account WHERE username = ? AND password = ? LIMIT 1'
    ...
    editPass: (user_id, password, new_password) => {
        return new Promise((resolve, reject) => {
            try {
                ...
                const sql = "SELECT * FROM account WHERE id = ? AND password = ? LIMIT 1"
    ...
    editMail: (user_id, pass, new_email) => {
        return new Promise((resolve, reject) => {
            ...
            try {
                password = md5(pass)
                const sql = "SELECT email, username FROM account WHERE id = ? AND password = ? LIMIT 1"
                db.query(sql, [user_id, password], (err, res) => {
                    if (err) return reject(err)
                    res = res[0]
                    if (res) {
                        user_name = res.username
                        //Gửi về mail cũ tin nhắn hoàn tác việc thay đổi này
                        //Kiểm tra mail trước đó có thay đã được thay đổi hay chưa
                        email_old = res.email
                        //Không thay đổi nếu mail không có sự thay đổi
                        ...
                        db.query(sql, [user_id], (err, res) => {
                            ...
                            res = res[0]
                            if (res) {
                                //Nếu chưa huỷ mail trước đó thì không gửi mail nữa 
                                //Thay đổi mail
                                const sql = "UPDATE account SET email = ?, status = 0 WHERE id = ?"
                                ...
                                token_edit_mail = jwt.sign({ data: email_old, iat: Date.now() }, 'shhhhh')
                                //Thêm vào token_edit_email
                                //Thêm token này vào sql
                                const sql = "INSERT INTO token_edit_email (value, user_id) VALUES (?, ?)"
                                db.query(sql, [token_edit_mail, user_id], async (err) => {
                                    if (err) return reject(err)
                                    //Gửi tin nhắn vào mail của bạn
                                    url2 = "http://localhost:3000/undo?token=" + token_edit_mail
                                    url = "http://localhost:3000/success?token=" + token_edit_mail
    ...
    //Hoàn tác lại mail khi bạn không phải là người thay đổi
    undoEmail: (token) => {
        return new Promise((resolve, reject) => {
            try {
                ...
                db.query(sql, [token], (err, res) => {
                    ...
                    if (res[0]) {
                        id = res[0].id
                        //Tiến hành đổi lại email trong tài khoản đó (và xác xác thực tài khoản luôn)
                        token_decode = jwt.decode(token)
                        //Có hiệu lực trong vòng 30 ngày
                        ...
                        if (time < die) {
                            ...
                            //Tiến hành hoàn tác lại mail cũ và xác tuhwujc tài khoản
                            const sql = "UPDATE account SET status = 1, email = ? WHERE id = ?"
