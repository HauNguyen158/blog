...
module.exports = {
    //Láy thông tin của bản thân
    profile: (user_id) => {
        return new Promise(async (resolve, reject) => {
            try {
                //Thống kê bài viết
                const sql = 'SELECT COUNT(*), user_id , substring(date,1,10) FROM posts WHERE user_id = ? GROUP BY user_id , substring(date,1,10)'
                db.query(sql, [user_id], (err, reponse) => {
                    ...
                    db.query(sql, [user_id], (err, reponse) => {
                        ...
                        //Lấy bài viết đã lưu (bài viết chưa đăng)
    ...
    acceptAddFriend: (id, user_id) => {
        return new Promise((resolve, reject) => {
            try {
                const sql = "UPDATE friend SET status = 1 WHERE user_id= ? AND user_id_2 = ?"
    ...
    removeFriend: (id, user_id) => {
        return new Promise((resolve, reject) => {
            try {
                const sql = "DELETE FROM friend WHERE (user_id = ? AND user_id_2 = ?) OR (user_id = ? AND user_id_2 = ?)"
